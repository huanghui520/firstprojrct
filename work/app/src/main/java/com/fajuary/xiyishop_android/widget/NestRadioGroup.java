...
public class NestRadioGroup extends LinearLayout {
     ...
     * 递归查找具有select属性的子控件
     */
    private static ImageView findSelectView(View child) {
        if (child instanceof ImageView)
            return (ImageView) child;
        if (child instanceof ViewGroup) {
            ViewGroup group = (ViewGroup) child;
            for (int i = 0, j = group.getChildCount(); i < j; i++) {
                ImageView check = findSelectView(group.getChildAt(i));
                if (check != null)
                    return check;
            }
        }
    ...
    public void addView(View child, int index, ViewGroup.LayoutParams params) {
        ...
        final ImageView imageView = findSelectView(child);
        if (view != null) {
            if (view.isChecked()) {
                mProtectFromCheckedChange = true;
                if (mCheckedId != -1) {
    ...
    private void setCheckedStateForView(int viewId, boolean checked) {
        ...
        View selectView=findViewById(viewId);
        if (checkedView != null && checkedView instanceof CompoundButton) {
            ((CompoundButton) checkedView).setChecked(checked);
        }
        if (selectView != null && selectView instanceof ImageView) {
            ((ImageView) selectView).setSelected(checked);
        }
//        if (checkedView != null && checkedView instanceof ImageView) {
//            ((ImageView) checkedView).setSelected(checked);
//        }
    }
